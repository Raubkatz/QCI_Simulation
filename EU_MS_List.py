"""
Country-level configuration definitions for national terrestrial QKD planning
scenarios.

This module defines a structured configuration format for country-specific
simulation parameters used in the QKD network planning workflow. Each
configuration describes one national scenario in terms of demographic scale,
geographic scale, endpoint counts, trusted repeater counts, center locations,
cluster sizes, spatial sampling parameters, and degree-distribution settings.

Purpose
-------
The module provides a reproducible and explicit parameter basis for generating
synthetic national QKD backbone models. It is intended for use by the main
simulation code, which imports these country definitions and applies them to
endpoint generation, graph construction, trusted repeater placement, and
reporting.

Contents
--------
1. ``CountryConfig``:
   Immutable dataclass storing all country-specific planning parameters.

2. Helper functions:
   - ``_scaled_endpoints``:
     Scales endpoint counts from the Austrian reference case using population.
   - ``_scaled_trns``:
     Scales trusted repeater counts from the Austrian reference case using
     geographic area.
   - ``_k2_k3_split``:
     Splits the total number of endpoints into degree-2 and degree-3 nodes.
   - ``_center_counts_from_weights``:
     Converts relative center weights into absolute center endpoint counts.

3. ``load_all_country_configs_from_module``:
   Builds and returns a dictionary of all national configurations.

4. ``COUNTRY_CONFIGS_EU``:
   Module-level dictionary containing the final loaded configuration set.

Modeling interpretation
-----------------------
Each country configuration is a planning-level approximation rather than a
deployment specification. The values encode assumptions about:
- how many QKD endpoint sites should be represented,
- how many trusted repeater nodes are needed,
- where endpoint clusters are centered,
- how cluster sizes are distributed,
- how strongly rural sampling should favor uniform territorial coverage versus
  center-based heavy-tailed spread,
- and how the degree distribution of the resulting graph should be defined.

The configurations are designed to be consumed directly by the simulation
workflow without additional manual preprocessing.
"""
...
class CountryConfig:
    """
    Immutable country-specific simulation parameter set.

    Each instance of this dataclass describes one national planning scenario for
    the terrestrial QKD simulation workflow. The fields combine demographic,
    geographic, topological, and spatial-sampling assumptions into one
    structured object.
    ...
    natural_earth_admin:
        Country name as used in the Natural Earth administrative boundary
        dataset. This name is used for geometry lookup.

    population_2025:
        Population value used as the planning reference for endpoint scaling and
        documentation.

    area_km2_2025:
        National area in square kilometers used as the planning reference for
        trusted repeater scaling and documentation.

    n_endpoints:
        Total number of simulated QKD endpoint sites for the country.

    k2_count:
        Number of endpoints assigned target degree 2.

    k3_count:
        Number of endpoints assigned target degree 3.

    n_trusted_repeater_nodes:
        Number of trusted repeater nodes used in the national planning case.

    n_double_trn_edges:
        Planning metadata for a double-TRN share or related reporting
        convention. This field is retained for consistency with earlier model
        versions and summary outputs.

    detour_factor:
        Multiplicative factor used to approximate route-corrected fiber length
        from geodesic distance.

    centers_lonlat:
        Mapping from center names to geographic longitude/latitude coordinates.

    center_counts:
        Absolute number of endpoints assigned to each configured center.

    center_radius_km:
        Sampling radius in kilometers for each center. A ``"__default__"``
        entry provides the fallback radius for centers without an explicit
        individual value.

    p_uniform_rural:
        Probability of drawing rural points from uniform country-wide sampling
        rather than center-based heavy-tailed sampling.

    heavy_tail_scale_km:
        Scale parameter for the heavy-tailed radial rural sampling model.

    heavy_tail_alpha:
        Shape parameter for the heavy-tailed radial rural sampling model.

    min_rural_point_distance_km:
        Minimum allowed spacing between rural points in variants of the model
        that enforce rural separation.

    min_rural_distance_to_centers_km:
        Minimum allowed distance between rural points and predefined centers in
        variants of the model that enforce center separation.
    """
    iso2: str
...
def _scaled_endpoints(population: int, pop_at: int = 9_197_213, n_ep_at: int = 250) -> int:
    """
    Scale the number of endpoints relative to the Austrian reference case.

    The function uses a simple proportional scaling rule based on population.
    Austria acts as the reference point, with ``pop_at`` corresponding to the
    Austrian population and ``n_ep_at`` to the Austrian endpoint count.

    A lower bound of 50 endpoints is enforced to avoid unrealistically small
    national backbone models for small countries.

    Parameters
    ----------
    population:
        Country population used for scaling.
    pop_at:
        Austrian reference population.
    n_ep_at:
        Austrian reference endpoint count.

    Returns
    -------
    int
        Scaled endpoint count, lower-bounded at 50.
    """
...
def _scaled_trns(area_km2: int, area_at: int = 83_882, n_tr_at: int = 50) -> int:
    """
    Scale the number of trusted repeater nodes relative to the Austrian
    reference case.

    The function uses a proportional scaling rule based on geographic area.
    Austria acts as the reference point, with ``area_at`` corresponding to the
    Austrian area and ``n_tr_at`` to the Austrian trusted repeater count.

    A lower bound of 5 trusted repeater nodes is enforced for nontrivial
    national planning cases.

    Parameters
    ----------
    area_km2:
        Country area used for scaling.
    area_at:
        Austrian reference area in square kilometers.
    n_tr_at:
        Austrian reference trusted repeater count.

    Returns
    -------
    int
        Scaled trusted repeater count, lower-bounded at 5.
    """
...
def _k2_k3_split(n_endpoints: int, frac_k2: float = 0.4) -> Tuple[int, int]:
    """
    Split the endpoint set into degree-2 and degree-3 nodes.

    The function assigns approximately ``frac_k2`` of all endpoints to degree 2
    and the remainder to degree 3. The resulting counts always sum exactly to
    ``n_endpoints``.
    ...
    frac_k2:
        Target fraction of degree-2 nodes.

    Returns
    -------
    Tuple[int, int]
        Pair ``(k2_count, k3_count)``.
    """
...
def _center_counts_from_weights(n_endpoints: int, weights: Dict[str, float]) -> Dict[str, int]:
    """
    Convert center weights into absolute endpoint counts.

    The function multiplies each weight by the total endpoint count and rounds
    to integers. If the rounded counts exceed the total number of endpoints,
    all counts are uniformly rescaled downward and rounded again.

    This function does not force the sum of center counts to equal the total
    endpoint count. The remaining endpoints, if any, are later available for
    rural sampling in the main simulation workflow.

    Parameters
    ----------
    n_endpoints:
        Total number of endpoints for the country.
    weights:
        Mapping from center names to relative center weights.

    Returns
    -------
    Dict[str, int]
        Absolute endpoint counts per center.
    """
...
def load_all_country_configs_from_module() -> Dict[str, CountryConfig]:
    """
    Build the complete dictionary of national QKD planning configurations.

    The function defines one ``CountryConfig`` per covered country and returns
    them as a dictionary keyed by the country display name.

    The national parameter choices follow a common modeling pattern:
    - endpoint counts are either set explicitly or scaled from Austria,
    - trusted repeater counts are either set explicitly or scaled from Austria,
    - degree-2 and degree-3 counts are derived through ``_k2_k3_split``,
    - center endpoint counts are derived from weight dictionaries,
    - center radii are set either explicitly per center or through a default
      fallback radius,
    - rural sampling parameters are adjusted to reflect country size and
      geography.

    Returns
    -------
    Dict[str, CountryConfig]
        Mapping from country names to configuration objects.
    """
    detour = 1.5
